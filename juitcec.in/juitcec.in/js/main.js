jQuery(document).ready(function($){
	...
	var y=1;
	var m,n,cls;
    $('.cd-qwerty').on('click', function(){
	y=$(this).val();
	console.log(y);
	switch(y){
	case 1:
		cls='.cd-section.qwerty0';
		break;
	case 2:
		cls='.cd-section.qwerty';
		break;
	case 3:
		cls='.cd-section.qwerty1';
		break;
	case 4:
		cls='.cd-section.qwerty2';
		break;
	case 5:
		cls='.cd-section.qwerty3';
		break;
	case 6:cls='.cd-section.qwerty5';
		break;
	case 7:cls='.cd-section.qwerty6';
		break;
	case 8:cls='.cd-section.qwerty7';
		break;	
	}
	/* var z=abs(x-y)*0.5;
	
        $('.cd-header').css({' -webkit-transition': '-webkit-transform' z +'s',
	 ' -moz-transition': '-moz-transform' z +'s',
	  ' transition': 'transform' z +'s';}),
	); */
   ...
   /*  $(document).keydown(function(event){
	...
	function nextSection(m,n) {
		if (!animating) {
			if ($('.cd-section.is-visible').next().length > 0) smoothScroll($(m),n);
		}
	}

	function prevSection(m,n) {
		if (!animating) {
			var prevSection = $(m);
			if(prevSection.length > 0 && $(window).scrollTop() != prevSection.offset().top) {
				smoothScroll(prevSection,n);
	...
	function smoothScroll(target,x) {
		animating = true;
        $('body,html').animate({'scrollTop': target.offset().top}, x*500, function(){ animating = false; });
	}
